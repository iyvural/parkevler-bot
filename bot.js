...
// Sadece bu numaralar cevap alır.
// Format: 90xxxxxxxxxx@s.whatsapp.net
const ALLOWED_USERS = [
...
// Railway volume kullanacaksan buna /data/auth_info ver
// Volume yoksa localde auth_info olarak çalışır
...
async function handleMessage(sock, msg) {
  ...
  // Tanımlı numara değilse hiç cevap verme
  ...
  if (['merhaba', 'selam', 'hi', 'hey', 'başla', 'basla', 'menu', 'menü', 'yardım', 'yardim', 'help'].includes(textLower)) {
    ...
    sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) {
          ...
          currentQR = null;
          console.log('Oturum logout oldu. Yeniden QR okutulmasi gerekir.');
        }
      }
    });
