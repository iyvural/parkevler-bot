...
function jidToPhone(jid) {
  if (!jid) return null;
  const match = String(jid).match(/^(\d{10,15})@/);
  return match ? match[1] : null;
}

function getSenderPhones(msg) {
  const values = [
    msg?.key?.remoteJid,
    msg?.key?.participant,
  ...
  for (const value of values) {
    const fromJid = jidToPhone(value);
    if (fromJid) phones.add(fromJid);

    const fromDigits = extractDigits(value);
    if (fromDigits && fromDigits.length >= 10 && fromDigits.length <= 15) {
      phones.add(fromDigits);
    }
  }
...
function isYetkiliUser(msg) {
  const phones = getSenderPhones(msg);
  const matchedPhone = phones.find((p) => ALLOWED_PHONES.includes(p));
...
async function handleMessage(sock, msg) {
  ...
  if (!auth.allowed) {
    console.log(
      `[${new Date().toISOString()}] ⛔ Yetkisiz kullanıcı: remoteJid=${auth.remoteJid} phones=${JSON.stringify(auth.phones)}`
    );
    return;
  }
